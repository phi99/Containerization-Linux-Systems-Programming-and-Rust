***In Progress**
	 		   ...
	 		   ----------------------------------------------------------
	     	 	         **  Implementing (Linux) Container in Rust **
	  		   ----------------------------------------------------------


```text
Container is basically an isolated process that could only see the process(es) that it creates (its child process(es)), has its own network stack, and seem to have its own filesystem with root directory not related to root on the host.
This isolated process is still being run by the same Linux Kernel of the host. Container is built out of Linux features such as chroot, namespaces, and cgroups. Namespaces controls what a process can see.
...
When linux system is started, it usually has a single namespace and a process is in one namespace of each type. However, additional namespaes could be created and assign process into them.
